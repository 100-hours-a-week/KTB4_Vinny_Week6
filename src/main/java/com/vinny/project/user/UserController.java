...
class UserController {
    ...
    public ResponseEntity<ApiResponse<UserResponse>> signUp(@Valid @RequestBody UserCreateRequest request){
        ...
        return ResponseEntity.status(200).body(ApiResponse.ok(new UserResponse(user)));
    }
    ...
    @PatchMapping("/{id}")
    public void updateUser(@PathVariable String id, @RequestBody UserPatchRequest request){
